...
public class LVQ3 extends LVQ {

	private static final double WINDOW = 0.60;
	private static final double EPSILON = 0.50;

	public LVQ3(Dataset dataset, double learningRate, int epochs, Class<? extends Distance> distanceClass, int codebookPerClass) {
		...
		for (int epoch = 0; epoch < epochs; ++epoch) {
			...
			for (Instance instance : trainSet) {
				...
				for (int i = 0; i < attributes.size(); ++i) {
					...
					if (sameClass(instance, bMU1) && sameClass(instance, bMU2)) {
						updateAttribute(bMU1, i, error1 * EPSILON);
						updateAttribute(bMU2, i, error2 * EPSILON);
					} else if (sameClass(instance, bMU1)) {
